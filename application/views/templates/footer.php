		
<footer class="footer">
	<p>Copyright &copy; <?= date('Y'); ?></p>
	<p>Feito por <a href="https://github.com/SilasRodrigues19" target="_blank" rel="noreferrer">Silas Rodrigues</a></p>
	<p>
		Distribuído por: 
		<a href="https://www.infinityfree.net" target="_blank" rel="noreferrer"><span class="iconify" data-icon="akar-icons:infinity"></span> 
			Infinity Free,
		</a>
		<a href="https://github.com" target="_blank" rel="noreferrer"><span class="iconify" data-icon="akar-icons:github-fill"></span> 
			GitHub
		</a>
	</p>
	<ul class="social">
		<hr>
		<li>
			<a href="https://github.com/SilasRodrigues19/WebResources/pulls" target="_blank" rel="noreferrer">
			Contribua com o projeto <span class="iconify" data-icon="ant-design:github-outlined"></span>
		</a>
		</li>
		<br>
		<li class="text-center text-muted">
			<p><span class="iconify" data-icon="bytesize:code"></span> Stack para devs</p>
			<p class="icons">
				<span data-toggle="tooltip" title="CodeIgniter"><span class="iconify" data-icon="simple-icons:codeigniter" data-width="20" data-height="20"></span></span>
				<span data-toggle="tooltip" title="MySQL"><span class="iconify" data-icon="fontisto:mysql" data-width="20" data-height="20"></span></span>
				<span data-toggle="tooltip" title="Bootstrap"><span class="iconify" data-icon="simple-icons:bootstrap" data-width="20" data-height="20"></span></span>
				<span data-toggle="tooltip" title="SASS"><span class="iconify" data-icon="cib:sass-alt" data-width="20" data-height="20"></span></span>
				<span data-toggle="tooltip" title="jQuery"><span class="iconify" data-icon="raphael:jquery" data-width="20" data-height="20"></span></span>
				<span data-toggle="tooltip" title="JavaScript"><span class="iconify" data-icon="akar-icons:javascript-fill" data-width="20" data-height="20"></span></span>
			</p>
		</li>
	</ul>
</footer>	
<a id="smoothScroll" href="#main">
		<span class="iconify arrow-up" data-icon="bytesize:chevron-top" data-inline="false"></span>
</a>
</main>
		<script src="<?= base_url('public/libraries/sweetalert2/sweetalert2.min.js'); ?>"></script>
		<script src="<?= base_url('public/libraries/datatable/dataTables.min.js'); ?>"></script>
		<script src="<?= base_url('public/libraries/jquery/jquery.min.js'); ?>"></script>

		<script src="<?= base_url('public/libraries/iconify/iconify.min.js'); ?>"></script>
		<script src="<?= base_url('public/libraries/bootstrap/bootstrap.bundle.min.js'); ?>"></script>
		<script src="<?= base_url('public/libraries/select2/select2.min.js'); ?>"></script>
		<script src="<?= base_url('public/js/script.js'); ?>"></script>
	</body>
</html>